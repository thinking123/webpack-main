...
// 设置 module export info ： 只对 Harmony dep 模式 ，commonjs 不需要设置
	...
	apply(compiler) {
		compiler.hooks.compilation.tap(
			...
			compilation => {
				...
				const cache = compilation.getCache("FlagDependencyExportsPlugin");//CacheFacade
					...
					(modules, callback) => {
						...
						asyncLib.each(
							...
							(module, callback) => {
								...
								if (!module.buildMeta || !module.buildMeta.exportsType) { // harmony exportsType === namespace
									if (exportsInfo.otherExportsInfo.provided !== null) {
										// It's a module without declared exports
										statNoExports++; //没有导出 ， 对于 commonjs exports 也是没有导出，只对 harmony (es6) module 能够有导出（export）
										exportsInfo.setHasProvideInfo();
										exportsInfo.setUnknownExportsProvided(); // 设置提供的未知导出
										return callback();
									}
								}
							...
							err => {
								...
								const processDependency = dep => {
									const exportDesc = dep.getExports(moduleGraph); // HarmonyExportSpecifierDependency ,
								 ...
								 * @param {ExportsSpec} exportDesc info , '{"exports":["usedFunction"],"priority":1,"terminalBinding":true}'
								 * @returns {void}
								 */
								const processExportsSpec = (dep, exportDesc) => {
									...
									if (exports === true) {
										...
										const mergeExports = (exportsInfo, exports) => {
											for (const exportNameOrSpec of exports) { /// exportNameOrSpec === string
												let name;
