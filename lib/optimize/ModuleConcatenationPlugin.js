...
class ModuleConcatenationPlugin {
			...
			compilation.hooks.optimizeChunkModules.tapAsync(
				...
				(allChunks, modules, callback) => {
					...
					for (const module of modules) {
						...
						const relevantExports = exportsInfo.getRelevantExports(undefined); // used in module exports
					...
					for (const currentRoot of relevantModules) { // canBeRoot
						...
						for (const imp of this._getImports(
							...
							candidates.add(imp); // import 的 module
						}
	 ...
	 */
	_getImports(compilation, module, runtime) { // 返回 module import module
		const moduleGraph = compilation.moduleGraph;
		const set = new Set();
	...
	_tryToAdd(
		...
		// import 'module' ， import 语句的 module
