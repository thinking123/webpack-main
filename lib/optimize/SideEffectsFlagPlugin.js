...
//  module 是否有 SideEffect ， 需要连接 modules
// 1. 在 package.json : 文件是否有sideeffect，如果有sideeffect，webpack 不会对文件的export 做treeshaking ， sideeffect 在package.json文件设置
// 2. module ast 语句 : // 开始 ast pasrser ，program 设置 sideEffects === undefined
						...
						if (
							...
							if (sideEffects !== undefined) {
								if (module.factoryMeta === undefined) {
									module.factoryMeta = {};
								}// 文件是否有sideeffect，如果有sideeffect，webpack 不会对文件的export 做treeshaking ， sideeffect 在package.json文件设置
				...
				if (this._analyseSource) {
					...
					const parserHandler = parser => {
						...
						parser.hooks.program.tap("SideEffectsFlagPlugin", () => {
							sideEffectsStatement = undefined; // 开始 ast pasrser ，program 设置 sideEffects === undefined
						});
						parser.hooks.statement.tap( // 语句是否有 sideEffects
								...
								switch (statement.type) {
									case "ExpressionStatement":
										if (
											...
											sideEffectsStatement = statement; // statement sideEffect
										}
										break;
									case "IfStatement":
									case "WhileStatement":
						...
						parser.hooks.finish.tap("SideEffectsFlagPlugin", () => {
							if (sideEffectsStatement === undefined) {
								parser.state.module.buildMeta.sideEffectFree = true; // no sideEffect
							} else {
					...
					modules => {
						...
						for (const module of modules) {
							if (module.getSideEffectsConnectionState(moduleGraph) === false) {
								const exportsInfo = moduleGraph.getExportsInfo(module);//没有sideeffect 可以做treeshaking
