			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = (parser, parserOptions) => {
						...
						(decl, statement) => {
							...
							if (
								...
								if (decl.init.type === "ClassExpression") {
									...
									const fn = InnerGraph.tagTopLevelSymbol(parser, name);
									declWithTopLevelSymbol.set(decl, fn); // TopLevelSymbol:{name} , 保存顶层var(TopLevelSymbol)
									if (
					...
					parser.hooks.declarator.tap("InnerGraphPlugin", (decl, statement) => {
						if (!InnerGraph.isEnabled(parser.state)) return;
						const fn = declWithTopLevelSymbol.get(decl);// TopLevelSymbol

						if (fn) {
							InnerGraph.setTopLevelSymbol(parser.state, fn); //设置顶层 var (TopLevelSymbol)
