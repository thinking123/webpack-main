...
exports.addUsage = (state, symbol, usage) => {
	...
	if (innerGraphState) {
		...
		const info = innerGraph.get(symbol); // TopLevelSymbol : {name}
		if (usage === true) {
			innerGraph.set(symbol, true);
		} else if (info === undefined) {
			innerGraph.set(symbol, new Set([usage])); //typeof  usage === string
		} else if (info !== true) {
			info.add(usage);
		}
	}
...
exports.inferDependencyUsage = state => {
	...
	// innerGraph 使用的 var key === TopLevelSymbol
...
exports.setTopLevelSymbol = (state, symbol) => {
	...
	if (innerGraphState) {
		innerGraphState.currentTopLevelSymbol = symbol; // 顶层var(TopLevelSymbol:{name})
	}
};
...
/** const declar: defineVariable , tagVariable
