 ...
 * @property {number=} minChunks // module 在 chunks 之间共享的最小次数
...
const compareEntries = (a, b) => {
	// 1. by priority ， 使用 priority 高的
	const diffPriority = a.cacheGroup.priority - b.cacheGroup.priority;
	if (diffPriority) return diffPriority;
	// 2. by number of chunks ，使用 chunks number 高的
	const diffCount = a.chunks.size - b.chunks.size;
	if (diffCount) return diffCount;
	// 3. by size reduction ，使用减少多的
	const aSizeReduce = totalSize(a.sizes) * (a.chunks.size - 1);
	const bSizeReduce = totalSize(b.sizes) * (b.chunks.size - 1);
	const diffSizeReduce = aSizeReduce - bSizeReduce;
	if (diffSizeReduce) return diffSizeReduce;
	// 4. by cache group index ， 使用 后面的cacheGroup
	const indexDiff = b.cacheGroupIndex - a.cacheGroupIndex;
	if (indexDiff) return indexDiff;
	// 5. by number of modules (to be able to compare by identifier)
	const modulesA = a.modules;
	const modulesB = b.modules; // 使用 包括module 多的
	const diff = modulesA.size - modulesB.size;
	if (diff) return diff;
	// 6. by module identifiers
	modulesA.sort();
	modulesB.sort(); // 按照 identifier 排序
...
/** 返回 {defaultSizeTypes Keys : value }
...
/** 删除 chunk之后 不能减少 SizeReduction ， size = sum(module)
 * @param {SplitChunksSizes} sizes the sizes
...
/** 获取 minSize > sizes 的 key ： [javascript]
 * @param {SplitChunksSizes} sizes the sizes
 * @param {SplitChunksSizes} minSize the min sizes
...
/** cacheGroups === {default:{idHint: "",reuseExistingChunk: true,	minChunks: 2,priority: -20	} }
 ...
 * @param {string[]} defaultSizeTypes the default size types === ["javascript" , "unknown"]
...
const normalizeCacheGroups = (cacheGroups, defaultSizeTypes) => {
	...
	if (typeof cacheGroups === "object" && cacheGroups !== null) {
		...
		const handlers = [];
		for (const key of Object.keys(cacheGroups)) { // key === default
			...
			} else {// split config 配置
				const source = createCacheGroupSource(option, key, defaultSizeTypes);
				handlers.push((module, context, results) => { // cacheGroups 校验 handler
					if (
						checkTest(option.test, module, context) &&// 校验  module name
						checkModuleType(option.type, module) &&// 校验  module type
						checkModuleLayer(option.layer, module)// 校验  module layer
					) { // module 的 config 配置
						results.push(source);
					}
				});
...
const createCacheGroupSource = (options, key, defaultSizeTypes) => {
	...
	const minSizeReduction = normalizeSizes(
		...
		defaultSizeTypes
	); // maxSize 是 maxAsyncSize 和 maxInitialSize 默认值
		...
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					...
					const ZERO = BigInt("0");
					const ONE = BigInt("1");
					const START = ONE << BigInt("31");// 1 (0 * 31)
					let index = START;
					for (const chunk of chunks) {
						chunkIndexMap.set(
							chunk, // 设置每个 chunk 的 key
							index | BigInt((Math.random() * 0x7fffffff) | 0)//0x7fffffff == 1 * 31 , 12.12 | 0 === 12
						);
						index = index << ONE;
					}
					/** 使用每个 chunk 的 key : key1 ^ key2 (key2 ^ key1 === key1 ^ key2)计算 key
					...
					const keyToString = key => {// chunk key from BigInt
						if (typeof key === "bigint") return key.toString(16);
						return chunkIndexMap.get(key).toString(16);
					};
					// 返回多个 chunks === chunkSetsInGraph (chunk 包括相同的module ，分组) ， 返回一个 chunk === singleChunkSets , module 一一对应 chunks,chunk , module 对应 chunkkey
					const getChunkSetsInGraph = memoize(() => {
						...
						for (const module of compilation.modules) {
							const chunks = chunkGraph.getModuleChunksIterable(module); // module -> chunks,chunk
						...
						return { chunkSetsInGraph, singleChunkSets }; // chunkSetsInGraph : 包含相同module 的 chunks，
					});
					...
					const groupChunksByExports = module => {
						...
						for (const chunk of chunkGraph.getModuleChunksIterable(module)) {
							const key = exportsInfo.getUsageKey(chunk.runtime); // 按照 exportInfo 分组 chunk
					...
					// (only smaller sets can be subset) 收集数量相同的chunkskey,chunkcount > 1
					...
					const createGetCombinations = (
						chunkSets,
						singleChunkSets,
						chunkSetsByCount // chunkSets 按照 size 分组
					) => {
						/** @type {Map<bigint | Chunk, (Set<Chunk> | Chunk)[]>} */
						const combinationsCache = new Map();
						// module -> key(chunkkey)
						return key => { // 每个 module 对应的key ： getKey ， key：包含 module 的 chunks
							const cacheEntry = combinationsCache.get(key);
							if (cacheEntry !== undefined) return cacheEntry;
							if (key instanceof Chunk) { // 只有一个chunk
								const result = [key];
					...
					/** map(chunks , map ( chunkFilter , { chunks:chunkFilter(chunks) , getKey(chunks) } ))
					 ...
					 *
					 *
					 * module 按照 属于的 cacheGroup.key 和 属于的chunks key 分组
					 *
					 * module 分组条件 minChunks , chunksFilter , test
					 *
					 *
					...
					const addModuleToChunksInfoMap = (
						...
						// Check if the name is ok , 添加的chunk 必须在 selectedChunks 所在的 group
						...
						const key = // 如果cacheGroup设置了 name ， 该 cacheGroup ， 会收集符合该 cahceGroup test 的所有 module 和 chunk
							cacheGroup.key +
							(name
								? ` name:${name}`
								: ` chunks:${keyToString(selectedChunksKey)}`); // module 属于相同的chunks，属于相同的 cacheGroup ， 一组
						// Add module to maps
						let info = chunksInfoMap.get(key);
						if (info === undefined) {
							chunksInfoMap.set( // 不设置 name  chunksInfoMap : module 肯定都在 selectedChunks 内 ，那么一个key内的 modules 都在 selectedChunks 内
								key,
								(info = {
						...
						if (info.modules.size !== oldSize) { // 计算 modules total size
							for (const type of module.getSourceTypes()) {
								info.sizes[type] = (info.sizes[type] || 0) + module.size(type); // module 文件 size ， 多个module 按照 module type 计算total size
							}
						}// 如果是 selectedChunksKey 只能是一个情况，不可能 oldChunksKeys(!= undefined) = chunksKeys
						// 如果是 name cacheGroup 提供了 name ，那么一个 cacheGroup 会分在一个 组
					...
					logger.time("modules");
					// 对每个module 对应的 cacheGroups 计算 chunk 添加到  chunksInfoMap （按照 cacheGroup 计算key ）
					// Walk through all modules
					for (const module of compilation.modules) {
						// Get cache group ，module test 获取 options
						let cacheGroups = this.options.getCacheGroups(module, context);// 获取 module 符合的cacheGroup
						...
						const getCombs = memoize(() => {
							const chunks = chunkGraph.getModuleChunksIterable(module); // module 属于的 chunk
							const chunksKey = getKey(chunks); // chunk ^ chunk key or chunk
						...
						for (const cacheGroupSource of cacheGroups) {
							...
							//对每个module 属于的chunk 进行分组Set ，从分组内获取当前module  对应的chunks 分组 ，该分组内的chunk 都包含了module ，chunk 可以继续分组如果 chunk内有其他module 可以得到类似的分组 [max chunks 包含module, 小于 max chunks ，但是chunks 之间存在 有其他module 交集存在的情况,module = 1 , chunk1:[1,2,3],chunk2:[1,2],chunk3:[1,3]],chunks = [chunk1,chunk2,chunk3] , [chunk1 , chunk2 对于module2] , [chunk1 , chunk3 对于module 3]
							...
							for (const chunkCombination of combs) {
								...
								// Select chunks by configuration : selectedChunks === filter chunks by chunksFilter  , selectedChunksKey === getKey(selectedChunks)
					...
					/** 删除 包含 sourceTypes 的 module  和 sizes
					...
					/** 删除不符合 minSize （cacheGroup 对应的size ： 所有属于该 cahceGroup的 module size ）的 module
					 * @param {ChunksInfoItem} info entry
					...
					const removeMinSizeViolatingModules = info => {
						...
						return info.modules.size === 0; // 如果删除不合符minsize 条件的 sizetype 之后 没有module ，删除 chunksInfoMap 对应的key
					};
					// 删除不符合 minSize ， minSizeReduction 的 chunksInfoMap key
					// Filter items were size < minSize
					for (const [key, info] of chunksInfoMap) {
						if (removeMinSizeViolatingModules(info)) {
							chunksInfoMap.delete(key);
						} else if ( // 从chunksInfoMap 对应的key 内的chunks 删除 modules 的（sizes） < minSizeReduction ,不能生成chunk
					...
					// 按照 cacheKey（minChunks)
					//chunksInfoMap 以 chunks 和 cacheKey（minChunks） 分组 每个 chunks 对应的 modules（在 chunks 都包含的modules ），
					// 分组是从 最小的 minChunks 向上分组的
					// chunk1=[a,1,2,3] , chunk2=[b ,1,2], chunk3 = [c , 1,3]
					//cacheGroup1{minChunks > 1} , cacheGroup2{minChunks > 2}
					//分组情况:cacheGroup2_1 modules : [1] , chunks:[chunk1,chunk2,chunk3]
					//分组情况:cacheGroup1_1 modules : [1, 2 ] , chunks:[chunk1,chunk2]
					//分组情况:cacheGroup1_2 modules : [1, 3] , chunks:[chunk1,chunk3]
					// 按照 name 分组
					while (chunksInfoMap.size > 0) {
						// Find best matching entry
						let bestEntryKey;
						let bestEntry;
						for (const pair of chunksInfoMap) { // 比较 cacheGroups 获取 bestEntry
							const key = pair[0];
							const info = pair[1];
							if (
								bestEntry === undefined ||
								compareEntries(bestEntry, info) < 0 //优先级排序，按照chunks 数量排序 ，按照sizes 大小排序 ，按照 cacheGroupIndex 排序 ， 按照modules 数量排序
							) { // 使用 优先级高的  info
								bestEntry = info;
								bestEntryKey = key;
							}
						...
						chunksInfoMap.delete(bestEntryKey); // 删除处理的 key

						let chunkName = item.name; // 是否设置了名称
						...
						if (chunkName) {  // 设置了名称
							...
							if (chunkByName !== undefined) {
								...
								isExistingChunk = item.chunks.size !== oldSize; // 从 chunks 选择 newChunk
							}
						} else if (item.cacheGroup.reuseExistingChunk) { // 没有设置名称， 从 item.chunks 找到合适的 chunk
							outer: for (const chunk of item.chunks) {
								// 从 chunks 获取 newChunk，chunk 只包含了所有的 modules，不是入口chunk ， chunk 有名称，名称length min
								if (
									chunkGraph.getNumberOfChunkModules(chunk) !==  // (chunk.modules  === modules) size
									item.modules.size
								) {
									continue;
								}
								if (
									item.chunks.size > 1 &&
									chunkGraph.getNumberOfEntryModules(chunk) > 0 // chunk 包括入口 module
								) {
									continue;
								}
								for (const module of item.modules) { // chunk是否包含了所有的 modules
							...
							if (newChunk) {
								item.chunks.delete(newChunk); // 删除 newChunk
								chunkName = undefined;
								isExistingChunk = true;
								isReusedWithAllModules = true;
							}
						}
						// 是否强制 enforceSizeThreshold
						// sizes > enforceSizeThreshold 强制 split
						// 否则需要计算 maxInitialRequests ， maxAsyncRequests
						const enforced =
						...
						// Check if maxRequests condition can be fulfilled ， chunk 所在的 group chunks 数量大于 maxInitialRequests|maxAsyncRequests删除 chunk
						if (
							!enforced &&
						...
						// items.modules 必须都在 chunks 内 , 不能存在 不包括 module  的 chunk
						outer: for (const chunk of usedChunks) { // usedChunks 包含所有的 item.modules
							for (const module of item.modules) { // 如果cacheGroup按照name 分组，需要删除不包括modules 的 chunk
								if (chunkGraph.isModuleInChunk(module, chunk)) continue outer;
							}
							usedChunks.delete(chunk);
						} // chunks 包括所有的 item.modules
						//[1,2][1,3][1, 2,3]=>[1,2,3]
						// Were some (invalid) chunks removed from usedChunks?
						// => readd all modules to the queue, as things could have been changed
						if (usedChunks.size < item.chunks.size) { // 删除不符合的chunks
							if (isExistingChunk) usedChunks.add(newChunk);
							if (usedChunks.size >= item.cacheGroup.minChunks) { // 重新计算
								const chunksArr = Array.from(usedChunks);
								for (const module of item.modules) {
									addModuleToChunksInfoMap( // 重新加载 ChunksInfoMap
										item.cacheGroup,
						...
						if (
							...
							for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
								if (!item.modules.has(module)) { // chunk 剩余 module
									for (const type of module.getSourceTypes()) {
										chunkSizes[type] =
											(chunkSizes[type] || 0) + module.size(type);
									}
								}
							}// 获取 < minRemainingSize 的 types ，chunk 剩余 module size <  minRemainingSize types
							const violatingSizes = getViolatingMinSizes(
								chunkSizes,
								item.cacheGroup.minRemainingSize
							);
							if (violatingSizes !== undefined) { //删除 < minRemainingSize 的 types 的 modules
						...
						newChunk.chunkReason =
							...
							(isReusedWithAllModules // 没有设置名称，从 chunks 获取 newChunk
						...
						if (!isReusedWithAllModules) { // 不是从 chunks 获取 newChunk
							// Add all modules to the new chunk
							for (const module of item.modules) { // 连接 新的chunk 和 module
								if (!module.chunkCondition(newChunk, compilation)) continue;
								// Add module to new chunk
								chunkGraph.connectChunkAndModule(newChunk, module);
								// Remove module from used chunks
								for (const chunk of usedChunks) {// 删除 之前的chunk 和module
									chunkGraph.disconnectChunkAndModule(chunk, module);
								}
							}
						} else { // 没有设置名称，chunks 包含所有的modules
						...
						) {//如果多个 cacheGroup合并到一个 chunk ，获取多个cacheGroup 属性的max
							const oldMaxSizeSettings = maxSizeQueueMap.get(newChunk); //设置了名称 newChunk 是唯一的，没有设置名称 只有从chunks获取才会有相同的chunk，此时需要合并多个cacheGroup
							maxSizeQueueMap.set(newChunk, {
								minSize: oldMaxSizeSettings
									? combineSizes( // 合并获取最大的 minSize
								...
								maxAsyncSize: oldMaxSizeSettings
									? combineSizes( // 合并最小的 maxAsyncSize
								...
								maxInitialSize: oldMaxSizeSettings
									? combineSizes( // 合并最小的 maxInitialSize
						...
						for (const [key, info] of chunksInfoMap) { // 删除其他 cacheGroup 相同的数据
							if (isOverlap(info.chunks, usedChunks)) { // usedChunks 包括 chunks chunk
								...
								for (const module of item.modules) {
									if (info.modules.has(module)) {
										// remove module
										info.modules.delete(module);// chunk 都包括 module
										// update size
								...
								if (updated) {
									if (info.modules.size === 0) { // 其他 cacheGroup 和 当前map 重叠 ，已经split
										chunksInfoMap.delete(key);
										continue;
									}
									if (   // 删除重叠的module 后，是否符合条件
					...
					for (const chunk of Array.from(compilation.chunks)) {
						...
						const results = deterministicGroupingForModules({
							minSize,
							maxSize: mapObject(maxSize, (value, key) => { // 获取minSize ，maxSize 的max value
								const minSizeValue = minSize[key];
							...
							getKey(module) {
								...
								const ident = cachedMakePathsRelative(module.identifier());// path 相对 context 路径 ： './webpack-test/utils/loader1.js!./webpack-test/src/commonjs-main.js'
								const nameForCondition =
									module.nameForCondition && module.nameForCondition();
								const name = nameForCondition // resource 相对context 路径
									? cachedMakePathsRelative(nameForCondition)
									: ident.replace(/^.*!|\?[^?!]*$/g, "");
								const fullKey =
									name +
									automaticNameDelimiter +
									hashFilename(ident, outputOptions); // 计算   ident hash
								const key = requestToId(fullKey);
								getKeyCache.set(module, key); // key === webpack-test_src_commonjs-main_js-ae5016b6
								return key;//'webpack-test_node_modules_y_js-89dce06a'
							},
							getSize(module) { // 获取 module 源码 size
