		...
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					...
					/** 使用每个 chunk 的 key : key1 ^ key2 计算 key
					...
					// 返回多个 chunks === chunkSetsInGraph (chunk 包括相同的module ，分组) ， 返回一个 chunk === singleChunkSets
					...
					for (const module of compilation.modules) {
						...
						const getCombs = memoize(() => {
							...
							const chunksKey = getKey(chunks); // chunk ^ chunk key
						...
						for (const cacheGroupSource of cacheGroups) {
							...
							const combs = cacheGroup.usedExports // module 包含的 chunks 数组[chunks1 , chunks2]
					...
					while (chunksInfoMap.size > 0) {
						...
						for (const [key, info] of chunksInfoMap) { // 删除其他 cacheGroup 相同的数据
							if (isOverlap(info.chunks, usedChunks)) { // usedChunks 包括 chunks chunk
								...
								if (updated) {
									if (info.modules.size === 0) { // 其他 cacheGroup 没有数据，删除
										chunksInfoMap.delete(key);
										continue;
									}
									if (
