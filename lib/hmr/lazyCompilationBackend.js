...
// req.url === '/lazy-compilation-using-/Users/a/github/webpack-main/hot/dev-server.js@/Users/a/github/zstack/local-test/src/reactjs/index.js'
	const requestListener = (req, res) => {
		const keys = req.url.slice(prefix.length).split("@"); // 更新的 [request]
		req.socket.on("close", () => {
			setTimeout(() => {
	...
	server.on("request", requestListener); // 获取请求 request

	let isClosing = false;
	/** @type {Set<import("net").Socket>} */
	const sockets = new Set();
	server.on("connection", socket => {// This event is emitted when a new TCP stream is established.
		sockets.add(socket);
		socket.on("close", () => {
	...
	server.on("listening", err => { // http server 启动后
		if (err) return callback(err);
		const addr = server.address();
		if (typeof addr === "string") throw new Error("addr must not be a string");
		const urlBase = // http://localhost:58124
		...
		callback(null, {
			...
			},//'/Users/a/github/webpack-main/webpack-test/lazy-compilation/index.js'
			module(originalModule) {
				const key = `${encodeURIComponent(
					originalModule.identifier().replace(/\\/g, "/").replace(/@/g, "_")
				).replace(/%(2F|3A|24|26|2B|2C|3B|3D|3A)/g, decodeURIComponent)}`;
				const active = activeModules.get(key) > 0;
				return { // client === ../hot/lazy-compilation-web/node
