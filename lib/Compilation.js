...
//  使用 loader 处理之后的resource 文件 ， ast 分析文件 ，获取 deps 等属性
	...
	seal(callback) {
		...
		while (this.hooks.optimizeDependencies.call(this.modules)) { // SideEffectsFlagPlugin , FlagDependencyUsagePlugin
			/* empty */
		}
					...
					this.codeGeneration(err => { // 编译 code to string （normalmodule）， 获取 module 的 runtimes 保存到 this.codeGenerationResults
						...
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							const cont = () => {
								...
								this.hooks.processAssets.callAsync(this.assets, err => {// no plugin , HtmlWebpackPlugin (删除已经生成的 assets) ， HotModuleReplacementPlugin（保存module hash  计算更新的json 和 js（更新的module code ） 文件）, production : TerserPlugin
									if (err) {
										return finalCallback(
				...
				for (const runtime of chunkGraph.getModuleRuntimes(module)) {
					...
					if (set.size === 0) {
						...
						if (memCache) {
							...
							chunkGraph.addModuleRuntimeRequirements(module, runtime, set); // 添加 runtimes to ChunkGraphModule , key === module
						}
					}
				}
			}
