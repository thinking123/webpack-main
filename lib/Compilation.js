...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {// no plugin
			...
			this.hooks.optimizeChunkModules.callAsync(// no plugin
				this.chunks,
				this.modules,
				err => {
					if (err) {
					...
					this.codeGeneration(err => { // 编译 code to string （normalmodule）
						if (err) {
						...
						// 编译code to string （runtime module）
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							const cont = () => {
								...
								this.hooks.processAssets.callAsync(this.assets, err => {// no plugin
									if (err) {
										return finalCallback(
	...
	processRuntimeRequirements({
		...
		for (const module of modules) {// 收集所有的module 的 rumtimes
...
This prevents using hashes of each other and should be avoided.`);
		...
		const processChunk = chunk => {
			...
			try {
				...
				chunk.hash = chunkHashDigest; // 计算chunk hash
		...
		this.hooks.fullHash.call(hash); // no plugin
