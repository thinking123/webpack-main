...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		this.processDependenciesQueue = new AsyncQueue({ // child list = [addModuleQueue,factorizeQueue,buildQueue]
	...
	createStatsOptions(optionsOrPreset, context = {}) {
		...
		if (typeof optionsOrPreset === "object" && optionsOrPreset !== null) {
			...
			/** @type {Partial<NormalizedStatsOptions>} */
			const options = {};
			this.hooks.statsNormalize.call(options, context);//DefaultStatsPresetPlugin
			return /** @type {NormalizedStatsOptions} */ (options);
		}
	}
	...
	createStatsFactory(options) {
		...
		this.hooks.statsFactory.call(statsFactory, options); // DefaultStatsFactoryPlugin
		return statsFactory;
	}

	createStatsPrinter(options) {
		const statsPrinter = new StatsPrinter();
		this.hooks.statsPrinter.call(statsPrinter, options);//DefaultStatsPrinterPlugin
		return statsPrinter;
	}
	...
	_addModule(module, callback) {
		...
		this._modulesCache.get(identifier, null, (err, cacheModule) => { // 获取 module 缓存
		...
		module.needBuild(
			...
			(err, needBuild) => {
				...
				if (!needBuild) {
					...
					this.hooks.stillValidModule.call(module); // no plugin
					return callback();
				}

				this.hooks.buildModule.call(module);// no plugin
				...
				module.build(
					...
					err => {
						...
						this._modulesCache.store(module.identifier(), null, module, err => {
							...
							this.hooks.succeedModule.call(module);//no plugin
							return callback();
						});
					}
				);
	...
	_processModuleDependencies(module, callback) {
		...
		const sortedDependencies = []; // 获取需要处理的deps getResourceIdentifier !== null 的deps 这些deps 才需要继续循环处理
		...
		const onDependenciesSorted = err => {
			...
			//item = {depedencies:[] , factory:NormalModuleFactory , originModule: 当前depedencies对应的module}
			for (const item of sortedDependencies) {
				inProgressTransitive++;
				this.handleModuleCreation(item, err => {
					// In V8, the Error objects keep a reference to the functions on the stack. These warnings &
					// errors are created inside closures titemhat keep a reference to the Compilation, so errors are
		...
		/** 获取需要处理的deps getResourceIdentifier !== null 的deps
		 * @param {Dependency} dep dependency
		 * @returns {void}
		 */
		const processDependencyForResolving = dep => {
			...
			if (resourceIdent !== undefined && resourceIdent !== null) {
				...
				if (factoryCacheKey === constructor) { // 二级缓存，factoryCacheKey , factoryCacheKey2
					...
					const factory = this.dependencyFactories.get(constructor);//NormalModuleFactory
					...
					if (factoryCacheKey2 === factory) {
						...
						factoryCacheKey = constructor; // constructor === HarmonyImportSideEffectDependency
						factoryCacheKey2 = factory; // map(constructor ,factory )
					}
				} // 按照 cacheKey 分组 处理依赖处理 deps
				...
				const cacheKey = // cacheKey === 'module./util/util-export'
					category === esmDependencyCategory
						? resourceIdent
						: `${category}${resourceIdent}`;
				let list = factoryCacheValue.get(cacheKey); //  map (cacheKey ,  list = [dep])
				if (list === undefined) {
	...
	handleModuleCreation(
		...
		// 分解 resource => NormalModule ,  import(`${xxx}.js`)=> ContextModule
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				// 分解模块之后（创建NormalModule 模块），添加模块NormalModule，到compilation.modules，使用ModuleGraphModule 关联 new module ， 使用 ModuleGraphConnection 连接originModule（issuer） 和 module （new），add 就是关联module 添加到compilation map
				this.addModule(newModule, (err, module) => {
					...
					//  使用 loader 处理之后的resource 文件 ， ast 分析文件 ，获取 deps 等属性
					this._handleModuleBuildAndDependencies(
						originModule,
						module,
						recursive,
...
//  使用 loader 处理之后的resource 文件 ， ast 分析文件 ，获取 deps 等属性
	_handleModuleBuildAndDependencies(originModule, module, recursive, callback) {
		...
		// loader 处理resource 获取 source ，处理ast 和文件依赖等
	...
	_factorizeModule(
		...
		} // 如果 factory === NormalModuleFactory，从resource === loader1!resource，以及rules 创建对应的 NormalModule
		factory.create(
			{
				contextInfo: {
			...
			(err, result) => {
				if (result) { // result.module === NormalModule , XXXModule
					// TODO webpack 6: remove
					// For backward-compat
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		this.hooks.addEntry.call(entry, options);//no plugin
		...
		this.addModuleTree(
			...
			(err, module) => {
				...
				this.hooks.succeedEntry.call(entry, options, module); //no plugin
	...
	seal(callback) {
		...
		if (this._backCompat) {
			for (const module of this.modules) {// module 设置为一个 chunkGraph
				ChunkGraph.setChunkGraphForModule(module, chunkGraph);
			}
		}

		this.hooks.seal.call(); // WarnCaseSensitiveModulesPlugin 比较module 是否url CaseSensitive equal

		this.logger.time("optimize dependencies");
		while (this.hooks.optimizeDependencies.call(this.modules)) { // SideEffectsFlagPlugin , FlagDependencyUsagePlugin(production)
			/* empty */
		}
		this.hooks.afterOptimizeDependencies.call(this.modules); // no plugin
		this.logger.timeEnd("optimize dependencies");

		this.logger.time("create chunks");
		this.hooks.beforeChunks.call(); // no plugin
		this.moduleGraph.freeze("seal");
		/** @type {Map<Entrypoint, Module[]>} */
		const chunkGraphInit = new Map();
		for (const [name, { dependencies, includeDependencies, options }] of this
			.entries) { // _addEntryItem
			...
			if (!options.dependOn && !options.runtime) {
				entrypoint.setRuntimeChunk(chunk); // dependOn ， runtime 不设置
			}
...
Remove the 'runtime' option from the entrypoint.`);
			...
			if (dependOn) {
				...
				for (const dependency of dependOnEntries) {
					connectChunkGroupParentAndChild(dependency, entry); // 连接 depondOn Chunk
				}
			} else if (runtime) {
				const entry = this.entrypoints.get(name);
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		} // 连接 module 依赖 ， 连接 module 和 chunk
		buildChunkGraph(this, chunkGraphInit);
		this.hooks.afterChunks.call(this.chunks); // no plugin
		this.logger.timeEnd("create chunks");

		this.logger.time("optimize");
		this.hooks.optimize.call(); // optimize undefined , no plugin

		while (this.hooks.optimizeModules.call(this.modules)) { // no plugin
			/* empty */
		}
		this.hooks.afterOptimizeModules.call(this.modules); // no plugin

		while (this.hooks.optimizeChunks.call(this.chunks, this.chunkGroups)) { // split plugin
			/* empty */
		}
		this.hooks.afterOptimizeChunks.call(this.chunks, this.chunkGroups);// no plugin

		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {// no plugin
			if (err) {
			...
			this.hooks.afterOptimizeTree.call(this.chunks, this.modules);// no plugin

			this.hooks.optimizeChunkModules.callAsync(// no plugin , production ， ModuleConcatenationPlugin (连接所有的harmony module ，连接chunk 和 module )
				this.chunks,
				this.modules,
				err => {
					if (err) {
					...
					this.hooks.afterOptimizeChunkModules.call(this.chunks, this.modules);// no plugin

					const shouldRecord = this.hooks.shouldRecord.call() !== false;// no plugin

					this.hooks.reviveModules.call(this.modules, this.records); // no used for record
					this.hooks.beforeModuleIds.call(this.modules);// no plugin
					this.hooks.moduleIds.call(this.modules); // set module id
					this.hooks.optimizeModuleIds.call(this.modules);// no plugin
					this.hooks.afterOptimizeModuleIds.call(this.modules);// no plugin

					this.hooks.reviveChunks.call(this.chunks, this.records);// no used for record
					this.hooks.beforeChunkIds.call(this.chunks);// no plugin
					this.hooks.chunkIds.call(this.chunks);// set chunk id
					this.hooks.optimizeChunkIds.call(this.chunks);// no plugin , prod , FlagIncludedChunksPlugin
					this.hooks.afterOptimizeChunkIds.call(this.chunks);// no plugin
					...
					this.hooks.optimizeCodeGeneration.call(this.modules);// no plugin , prod : MangleExportsPlugin （设置 exportInfo  usedName）
					this.logger.timeEnd("optimize");

					this.logger.time("module hashing");
					this.hooks.beforeModuleHash.call();// no plugin
					this.createModuleHashes(); // 计算 module hash
					this.hooks.afterModuleHash.call();// no plugin
					this.logger.timeEnd("module hashing");

					this.logger.time("code generation");
					this.hooks.beforeCodeGeneration.call();// no plugin
					this.codeGeneration(err => { // 编译 code to string （normalmodule）， 获取 module 的 runtimes 保存到 this.codeGenerationResults
						if (err) {
							return finalCallback(err);
						}
						this.hooks.afterCodeGeneration.call();// no plugin
						this.logger.timeEnd("code generation");

						this.logger.time("runtime requirements");
						this.hooks.beforeRuntimeRequirements.call();// no plugin
						this.processRuntimeRequirements(); // 合并 modules runtime （runtime 对应的hooks 添加的runtime）to chunk runtimes
						this.hooks.afterRuntimeRequirements.call();// no plugin
						this.logger.timeEnd("runtime requirements");

						this.logger.time("hashing");
						this.hooks.beforeHash.call();// no plugin
						const codeGenerationJobs = this.createHash(); // run time jobs 计算 runtime  code
						this.hooks.afterHash.call();// no plugin
						this.logger.timeEnd("hashing");
						// 编译code to string （runtime module）, 添加 runtime module 代码到 codeGenerationResults
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							if (shouldRecord) {
								...
								this.hooks.recordHash.call(this.records); // no plugin
							...
							this.hooks.beforeModuleAssets.call(); // no plugin
							this.createModuleAssets();
							this.logger.timeEnd("module assets");

							const cont = () => {
								this.logger.time("process assets");
								this.hooks.processAssets.callAsync(this.assets, err => {// no plugin , HtmlWebpackPlugin (删除已经生成的 assets) ， HotModuleReplacementPlugin（保存module hash  计算更新的json 和 js（更新的module code ） 文件）, production : TerserPlugin , devtool sourcemap: SourceMapDevToolPlugin
									if (err) {
										return finalCallback(
											makeWebpackError(err, "Compilation.hooks.processAssets")
										);
									}
									this.hooks.afterProcessAssets.call(this.assets);// no plugin
	...
	Make sure to select an appropriate stage from Compilation.PROCESS_ASSETS_STAGE_*.`
										...
										// 添加module 依赖文件到 webpack 	this.fileDependencie ...
									this.summarizeDependencies();
									if (shouldRecord) {
										this.hooks.record.call(this, this.records);// no plugin
									}

									if (this.hooks.needAdditionalSeal.call()) {// no plugin
										this.unseal();
										return this.seal(callback);
									}
									return this.hooks.afterSeal.callAsync(err => {// no plugin
										if (err) {
											return finalCallback(
							...
							if (this.hooks.shouldGenerateChunkAssets.call() !== false) {// no plugin
								this.hooks.beforeChunkAssets.call();// no plugin
								this.createChunkAssets(err => {  // 生成文件string 保存 map
	...
	_codeGenerationModule(
		...
		runtimeTemplate,
		errors,
		results, // this.codeGenerationResults
		callback
	) {
	...
	processRuntimeRequirements({
		...
		chunkGraphEntries = this._getChunkGraphEntries() // runtime chunk
	} = {}) {
		...
		for (const module of modules) {// 收集所有的module 的 rumtimes , // 添加 runtimes to ChunkGraphModule , key === module
			if (chunkGraph.getNumberOfModuleChunks(module) > 0) {
				...
				for (const runtime of chunkGraph.getModuleRuntimes(module)) {
					...
					additionalModuleRuntimeRequirements.call(module, set, context); // no plugin

					for (const r of set) {
						const hook = runtimeRequirementInModule.get(r); // runtime hooks
					...
					if (set.size === 0) {
						...
						if (memCache) {
							...
							chunkGraph.addModuleRuntimeRequirements(module, runtime, set); // 添加 runtimes to ChunkGraphModule , key === module ,  添加module ,ChunkGraphModule 对应runtime的 runtimes , {[runtime]} : runtimes
						}
					}
				}
			}
		...
		for (const chunk of chunks) { // 获取 chunk的所有runtimes , chunkGraphChunk.runtimeRequirements = [runtimes]
			const set = new Set();
			for (const module of chunkGraph.getChunkModulesIterable(chunk)) { // 合并 所有的 runtimes to chunk runtimes
			...
			for (const r of set) {
				this.hooks.runtimeRequirementInChunk.for(r).call(chunk, set, context);//no plugin
			}
			// 从module 收集的runtimes 设置到chunk runtimes
			chunkGraph.addChunkRuntimeRequirements(chunk, set);
		}
		this.logger.timeEnd("runtime requirements.chunks");

		this.logger.time("runtime requirements.entries");
		for (const treeEntry of chunkGraphEntries) { // 获取 entry group（包括child chunk）的 所有 runtimes add to chunkGraphChunk.runtimeRequirementsInTree = [runtimes]
			const set = new Set();
			for (const chunk of treeEntry.getAllReferencedChunks()) { // 获取 包括 child chunks
				const runtimeRequirements =
			...
			this.hooks.additionalTreeRuntimeRequirements.call(
				...
				//从 runtime （string[]），添加 runtime 对应的 module
			for (const r of set) {
			...
			chunkGraph.addTreeRuntimeRequirements(treeEntry, set); // add runtime to runtimeRequirementsInTree
		}
	...
	addRuntimeModule(chunk, module, chunkGraph = this.chunkGraph) {
		...
		// Call hook
		this.hooks.runtimeModule.call(module, chunk);//no plugin
	}
	...
	/** 根据module 的 outgoingConnections 获取 依赖的 refmodule （module import 的 refmodule），当前module depth ，依赖按照import 顺序 depth + 1 ，然后处理依赖的依赖
	...
	assignDepths(modules) {
		...
		const queue = new Set(modules);//[m1,m2,1,m1->m3,m1->m4,m2->m5,m2->m6]
		queue.add(1);
		let depth = 0;
	...
	/** dep 引用的导出, import {usedFunction} from 'xxx' , return [usedFunction]
	...
	// module 的 hash ，通过计算 module (build code hash) 和 依赖的module 的 exports的hash ， 和 依赖的dependencies 和presentationalDependencies 的 hash
	_createModuleHash(
		module,
		chunkGraph,
		runtime,
	...
	createHash() {
		...
		for (const c of this.chunks) {
			if (c.hasRuntime()) {
				unorderedRuntimeChunks.push(c); // runtime chunk
			} else {
				otherChunks.push(c);
			}
		}
...
This prevents using hashes of each other and should be avoided.`);
		...
		// chunkhash and contenthash
		const processChunk = chunk => {
			// Last minute module hash generation for modules that depend on chunk hashes
			this.logger.time("hashing: hash runtime modules");
			const runtime = chunk.runtime;
			for (const module of chunkGraph.getChunkModulesIterable(chunk)) {
				if (!chunkGraph.hasModuleHashes(module, runtime)) { // runtime module 需要计算hash
			...
			const chunkHash = createHash(hashFunction);// 分别计算 content hash  和 chunk hash
			try {
				if (outputOptions.hashSalt) {
					chunkHash.update(outputOptions.hashSalt);
				}
				chunk.updateHash(chunkHash, chunkGraph); // 使用 modules 的hash , request : 更新 chunk hash
				this.hooks.chunkHash.call(chunk, chunkHash, { // JavascriptModulesPlugin 计算 bootstrap 代码 对应的 hash
					chunkGraph,
				...
				hash.update(chunkHashDigest); // 计算 compliation hash
				chunk.hash = chunkHashDigest; // 计算chunk hash (所有 module hash  和 bootstrap 代码 hash)
		...
		this.hooks.fullHash.call(hash); // no plugin , HotModuleReplacementPlugin
		this.fullHash = /** @type {string} */ (hash.digest(hashDigest));
		this.hash = this.fullHash.substr(0, hashDigestLength); // compliation hash ，所有 chunk hash 计算
		...
		for (const chunk of fullHashChunks) {
			...
			chunkHash.update(this.hash);// 更新 hash ChunkFullHashModule
			const chunkHashDigest = /** @type {string} */ (
				chunkHash.digest(hashDigest)
			);
	...
	emitAsset(file, source, assetInfo = {}) {
		...
		this.assets[file] = source; // CachedSource
	...
	createChunkAssets(callback) {
		...
		asyncLib.forEachLimit(
			...
			(chunk, callback) => {
				...
				let manifest;
				try {
					manifest = this.getRenderManifest({ // 获取 render function  和 config
						chunk,
						hash: this.hash,
						...
						assetCacheItem.get((err, sourceFromCache) => {
							...
							try {
								if ("filename" in fileManifest) {
									...
									const pathAndInfo = this.getPathWithInfo( // 获取文件名称
										filenameTemplate, // '[name].[contenthash].js'
								...
								this.hooks.chunkAsset.call(chunk, file); // no plugin
	...
	/** 运行module js 文件 （在js文件执行（vm.runInContext））
	 * @param {Module} module the module
	...
	executeModule(module, options, callback) {
		...
		processAsyncTree(
			...
			(module, push, callback) => {
				this.buildQueue.waitFor(module, err => { // wait module 依赖module build 完成
					if (err) return callback(err);
					this.processDependenciesQueue.waitFor(module, err => { // wait module 依赖module processDependenciesQueue 完成
						if (err) return callback(err);
						for (const { module: m } of this.moduleGraph.getOutgoingConnections( // 获取依赖 module （import module）
							module
						)) {
							const size = modules.size;
							modules.add(m);
							if (modules.size !== size) push(m); // 处理 依赖 循环
						}
						callback();
					});
				});
			...
			err => {
				...
				asyncLib.eachLimit(modules, 10, codeGen, err => {
					...
					this.chunkGraph = chunkGraph;
					this.processRuntimeRequirements({ // add aruntime module
						chunkGraph,
						modules,
						chunks,
						codeGenerationResults,
