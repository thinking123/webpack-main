...
//  使用 loader 处理之后的resource 文件 ， ast 分析文件 ，获取 deps 等属性
	...
	seal(callback) {
		...
		while (this.hooks.optimizeDependencies.call(this.modules)) { // SideEffectsFlagPlugin
			/* empty */
		}
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {// no plugin
			...
			this.hooks.optimizeChunkModules.callAsync(// no plugin , production
				this.chunks,
				this.modules,
				err => {
					if (err) {
					...
					this.hooks.optimizeChunkIds.call(this.chunks);// no plugin
					...
					this.hooks.optimizeCodeGeneration.call(this.modules);// no plugin
					...
					this.codeGeneration(err => { // 编译 code to string （normalmodule）， 获取 module 的 runtimes 保存到 this.codeGenerationResults
						...
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							const cont = () => {
								...
								this.hooks.processAssets.callAsync(this.assets, err => {// no plugin , HtmlWebpackPlugin (删除已经生成的 assets) ， HotModuleReplacementPlugin（保存module hash  计算更新的json 和 js（更新的module code ） 文件）
									if (err) {
										return finalCallback(
	...
	Make sure to select an appropriate stage from Compilation.PROCESS_ASSETS_STAGE_*.`
							...
							if (this.hooks.shouldGenerateChunkAssets.call() !== false) {// no plugin
								...
								this.
								createChunkAssets(err => {  // 生成文件string 保存 map
