...
class NamedModuleIdsPlugin {
	...
	/** setModuleId , set ChunkGraphModule . name === request relative to context value
	 * Apply the plugin
			...
			compilation.hooks.moduleIds.tap("NamedModuleIdsPlugin", modules => {
				...
				const unnamedModules = assignNames(
					Array.from(modules).filter(module => {
						...
						if (chunkGraph.getNumberOfModuleChunks(module) === 0) return false; // module 属于 chunk
						return chunkGraph.getModuleId(module) === null;// ChunkGraphModule.id
					}),
					...
					(m, name) => chunkGraph.setModuleId(m, name) // set ChunkGraphModule.id
				);
