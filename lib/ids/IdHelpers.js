...
const getShortModuleName = (module, context, associatedObjectForCache) => {
	const libIdent = module.libIdent({ context, associatedObjectForCache });//相对context 路径
...
const getShortChunkName = (
	...
	const chunkName = Array.from(chunk.idNameHints) // idHint + RootModules.request list
		.concat(shortModuleNames)
		.filter(Boolean)
		.join(delimiter);//stringLength > 100 : name + "delimiter" + hash(name)
...
const assignNames = (
	items,
	getShortName,
	getLongName,// getShortName 重复的时候使用
	comparator,
	usedIds,
	assignName
) => {
	...
	for (const [name, items] of nameToItems2) {
		if (!name) {
			...
			assignName(items[0], name); // 设置name
			usedIds.add(name);
		} else {
