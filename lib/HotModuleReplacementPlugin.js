	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				let hotIndex = 0;
				const fullHashChunkModuleHashes = {}; // FullHashChunkModule hash
				const chunkModuleHashes = {};// chunk 对应 module 的 hash
			// 执行tap [ fullHash , processAssets , record]
				compilation.hooks.record.tap(
					...
					(compilation, records) => {
						...
						records.fullHashChunkModuleHashes = fullHashChunkModuleHashes; //保存上次的 hash
						records.chunkModuleHashes = chunkModuleHashes;//保存上次的 hash
						records.chunkHashes = {};
						...
						for (const chunk of compilation.chunks) {
							records.chunkModuleIds[chunk.id] = Array.from( //保存 chunk module id
				...
				const updatedModules = new TupleSet();// 比较上次编译 hash 保存更新的 module
				/** @type {TupleSet<[Module, Chunk]>} */
				const fullHashModules = new TupleSet();
				/** @type {TupleSet<[Module, RuntimeSpec]>} */
				const nonCodeGeneratedModules = new TupleSet();
				compilation.hooks.fullHash.tap("HotModuleReplacementPlugin", hash => { // 保存 编译的 module hash
					const chunkGraph = compilation.chunkGraph;
					...
					for (const chunk of compilation.chunks) {
						...
						if (modules !== undefined) {
							if (records.chunkModuleHashes) { // 上次编译保存 module hahs
								if (fullHashModulesInThisChunk !== undefined) {
									for (const module of modules) {
										...
										if (
											...
											if (records.chunkModuleHashes[key] !== hash) {
												updatedModules.add(module, chunk); // 比较上次编译 hash 保存更新的 module
											}
					...
					() => {
						...
						for (const key of Object.keys(records.chunkHashes)) {
							...
							for (const id of records.chunkModuleIds[key]) {
								...
								if (module === undefined) {
									completelyRemovedModules.add(id); // 删除的module
								} else {
									remainingModules.push(module); // 不需要删除的module
								}
							}
							...
							if (currentChunk) {
								...
								newModules = chunkGraph // 新的module
									.getChunkModules(currentChunk)
									.filter(module => updatedModules.has(module, currentChunk));
								newRuntimeModules = Array.from( // 新的 runtime module
							...
							if (
								...
								} // add 需要更新的 module （newModules）to HotUpdateChunk
								...
								for (const entry of renderManifest) {
									...
									if ("filename" in entry) {
										...
										({ path: filename, info: assetInfo } = // filename === 'main.6fdcabb7e2dc8f370869.hot-update.js' ， 需要更新的 chunk id {moduleId(更新的module)}
											compilation.getPathWithInfo(
									...
									compilation.additionalChunkAssets.push(filename); // no plugin
...
To fix this, make sure to include [runtime] in the output.hotUpdateMainFilename option, or use the default config.`)
						...
						for (const [
							...
							const hotUpdateMainJson = {
								c: Array.from(updatedChunkIds), // 更新的 chunkId
								r: Array.from(removedChunkIds),//删除的 chunkId
								m:    													 // 删除的 module
							...
							compilation.emitAsset(filename, source, { // 保存 main.4c5af34f123cb4152c97.hot-update.json 文件
								hotModuleReplacement: true,
								...assetInfo
							});
						}
