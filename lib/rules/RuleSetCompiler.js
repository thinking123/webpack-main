...
class RuleSetCompiler {
	...
	compile(ruleSet) {
		const refs = new Map();
		const rules = this.compileRules("ruleSet", ruleSet, refs); //rules=[{...,rules:[....]}]
	...
	compileRule(path, rule, refs) {
		const unhandledProperties = new Set(// rule value !=== undeinfed
			Object.keys(rule).filter(key => rule[key] !== undefined)
		);
			// 每个rule 对应一个 compiledRule
		/** @type {CompiledRule} */
		const compiledRule = {
			conditions: [], // 插入 rule 编译后条件 [ { property:rule[key],matchWhenEmpty,fn } ]
			effects: [],
			rules: undefined,
			oneOf: undefined
		};
