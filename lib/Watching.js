...
class Watching {
	...
	_go(fileTimeInfoEntries, contextTimeInfoEntries, changedFiles, removedFiles) {
		...
		const run = () => {
			...
			this.compiler.hooks.watchRun.callAsync(this.compiler, err => { // no plugin , webpack-dev-server
				if (err) return this._done(err);
				const onCompiled = (err, compilation) => { // 编译完成，代码生成到compilation assets
					if (err) return this._done(err, compilation);
					if (this.invalid) return this._done(null, compilation);

					if (this.compiler.hooks.shouldEmit.call(compilation) === false) { // no plugin
						return this._done(null, compilation);
					}

					process.nextTick(() => {
						const logger = compilation.getLogger("webpack.Compiler");
						logger.time("emitAssets"); // 生成代码到文件
						this.compiler.emitAssets(compilation, err => {
							...
							this.compiler.emitRecords(err => {
								...
								if (compilation.hooks.needAdditionalPass.call()) {
									...
									this.compiler.hooks.done.callAsync(stats, err => {// no plugin , webpack-dev-server , 完成编译
		...
		this.watcher = this.compiler.watchFileSystem.watch(
			...
			(fileName, changeTime) => {
				if (!this._invalidReported) {
					this._invalidReported = true;
					this.compiler.hooks.invalid.call(fileName, changeTime);// no plugin , webpack-dev-server
				}
				this._onInvalid();
			}
		);
