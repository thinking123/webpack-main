...
module.exports = class HarmonyImportDependencyParserPlugin {
	...
	apply(parser) {
		...
		parser.hooks.isPure
			...
			});// 只是import了，但是代码没有使用：HarmonyImportSideEffectDependency
		parser.hooks.import.tap( // import {fun} from './other.js' ，执行import 语句的tap
			"HarmonyImportDependencyParserPlugin",
			(statement, source) => {// lastHarmonyImportOrder 从 1 开始计数
		...
		);// import {fun} from './other.js'  , fun 语句的tap ， hook import 的 var
		parser.hooks.importSpecifier.tap(
			"HarmonyImportDependencyParserPlugin",
			(statement, source, id, name) => {// import {util1 as util2} from 'fs.js'
				const ids = id === null ? [] : [id];//id === util1
				parser.tagVariable(name, harmonySpecifierTag, {//用tag 标记 hook ， callHooksForInfoWithFallback ，call tag ， call name
					name,//name === util2
					source, // === fs.js
					ids,
		...
		parser.hooks.expression
			...
			.tap("HarmonyImportDependencyParserPlugin", expr => {
				const settings = /** @type {HarmonySettings} */ (parser.currentTagData);// currentTagData
		...
		parser.hooks.expressionMemberChain
			...
			.tap("HarmonyImportDependencyParserPlugin", (expr, members) => {
				...
				InnerGraph.onUsage(parser.state, e => (dep.usedByExports = e));
				return true;
			});// import 语句的 var， 在代码内使用 HarmonyImportSpecifierDependency：import {b} from './b.js' ; b() // 在代码内使用 ,在callMember（调用使用：b())
