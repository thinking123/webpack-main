...
const visitModules = (
	...
	for (const [chunkGroup, modules] of inputEntrypointsAndModules) {
		...
		const chunkGroupInfo = {
			...
			preOrderIndex: 0,
			postOrderIndex: 0,
			chunkLoading: // chunkLoading : jsonp ... , asyncChunks: boolean
		...
		if (chunkGroup.getNumberOfParents() > 0) {// dependOn entry
	...
	const processConnectQueue = () => {
		...
		for (const [chunkGroupInfo, targets] of queueConnect) {//targets === [newchunkGroupInfo]
	...
	const processChunkGroupsForMerging = () => {
		...
		// Execute the merge
		for (const info of chunkGroupsForMerging) { // [newchunkGroupInfo]
	...
	const processOutdatedChunkGroupInfo = () => {
		...
		for (const info of outdatedChunkGroupInfo) {//newChunkGroupInfo
	...
	while (queue.length || queueConnect.size) {
		...
		if (chunkGroupsForCombining.size > 0) { // dependOn
...
const connectChunkGroups = (
	...
	for (const [block, connections] of blockConnections) { // import()  : [AsyncDependenciesBlock,]
...
const buildChunkGraph = (compilation, inputEntrypointsAndModules) => {
	...
	visitModules( // 利用 OutgoingConnections 获取 module 依赖的其他modules（从module require 或者 import 的其他 modules）, 对 import() 会创建 chunk，chunkGroup
		logger,
		compilation,
		inputEntrypointsAndModules,
		chunkGroupInfoMap,
