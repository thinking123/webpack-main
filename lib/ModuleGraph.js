...
// originModule.outgoingConnections module.incomingConnections
...
class ModuleGraph {
	...
	_getModuleGraphModule(module) {
		let mgm = this._moduleMap.get(module);//module key
		if (mgm === undefined) {
	...
	/**module.incomingConnections(connection)  ,originModule.outgoingConnections.push(connection)
	 * @param {Module} originModule the referencing module
	 * @param {Dependency} dependency the referencing dependency
	 * @param {Module} module the referenced module
	 * @returns {void}
	 */
	setResolvedModule(originModule, dependency, module) { // dependency 是 originModule 的deps
	 ...
	 * @returns {void}
	 */
	updateModule(dependency, module) { // 连接 x -> b -> c : x -> c ,  import x from 'x' , //b.js export * from 'c' ,//c.js export fun
		const connection = this.getConnection(dependency);
		if (connection.module === module) return;
		const newConnection = connection.clone();
		newConnection.module = module; // 修改 module : b -> c
		this._dependencyMap.set(dependency, newConnection);
		connection.setActive(false);
		const originMgm = this._getModuleGraphModule(connection.originModule);
		originMgm.outgoingConnections.add(newConnection); //连接 x -> c
	 ...
	 * @returns {Module} the referenced module
	 */
	getModule(dependency) {// 返回 dep 对应的 mod ， 包括dep 代码的module
		const connection = this.getConnection(dependency);
	 ...
	 */
	freeze(cacheStage) { // cacheStage === seal or
