...
// [{loader:loader1, options:op1}] => loader1??ident?op1|resurl
...
/** xxx?bbb === {loader:xxx , options: bbb}
...
// RuleSetCompiler apply all plugins list
const ruleSetCompiler = new RuleSetCompiler([
	new BasicMatcherRulePlugin("test", "resource"),//(ruleKey , dataProperty ?? ruleKey in data) for test ruleValue
...
class NormalModuleFactory extends ModuleFactory {
	...
	constructor({
		...
		fs,
		resolverFactory,
		options, // options === this.options.module
		...
		// 解析 str : http://www.qq.com?sfsdf#dsddsfsdf , [http://www.qq.com,?sfsdf,#dsddsfsdf]
		const cacheParseResource = parseResource.bindCache(
			associatedObjectForCache // associatedObjectForCache === compiler
		);
		// 分解文件 ， request -> NormalModule
		this.hooks.factorize.tapAsync(
			{
				name: "NormalModuleFactory",
				stage: 100
			},
			(resolveData, callback) => {// resolve 拿到resolve request 文件数据
				this.hooks.resolve.callAsync(resolveData, (err, result) => {
					if (err) return callback(err);
				// resolveData.createData 会拿到resource  和 loader 路径，以及对应的 rule effect 合并
					// Ignored
					...
					this.hooks.afterResolve.callAsync(resolveData, (err, result) => {// no plugin
						if (err) return callback(err);
						...
						this.hooks.createModule.callAsync(// no plugin
							createData,
							resolveData,
							(err, createdModule) => {
								if (!createdModule) {
		...
		); // 处理 request 获取 ： request path loaders ， rule 对应的loaders ，resolve request 文件数据 ， 保存数据 data.createData
		this.hooks.resolve.tapAsync(
			...
			(data, callback) => {
				...
				const loaderResolver = this.getResolver("loader"); // resolver loader
				...
				let scheme = getScheme(request); // htmlplugin request === 'data:text/javascript,__webpack_public_path__ = __webpack_base_uri__ = htmlWebpackPluginPublicPath;'
//'/Users/a/github/webpack-main/webpack-test/src/index.css.webpack[javascript/auto]!=!!!/Users/a/github/webpack-main/node_modules/css-loader/dist/cjs.js??ruleSet[1].rules[2].use[1]!/Users/a/github/webpack-main/webpack-test/src/index.css'
				if (!scheme) {
					/** @type {string} */
					let requestWithoutMatchResource = request;
					const matchResourceMatch = MATCH_RESOURCE_REGEX.exec(request); // 不包括 ! , ...!=! resourceurl!=!loaderjs
					if (matchResourceMatch) { // xxx!=!xxx request
					...
					if (!scheme && !contextScheme) {
						...
						noPreAutoLoaders = firstChar === 45 && secondChar === 33; // startsWith "-!" // "!".charCodeAt(0) === 33 , "-".charCodeAt(0) === 45
						noAutoLoaders = noPreAutoLoaders || firstChar === 33; // startsWith "!"
						noPrePostAutoLoaders = firstChar === 33 && secondChar === 33; // startsWith "!!";
						const rawElements = requestWithoutMatchResource // 删除前缀! 或者 !! ， 获取loaders 和 request , rawElements =[loader1,loader2,request]
							.slice(
							...
							.split(/!+/);//删除 start `-!` 按照 ! a!b ===[a,b] , a 是loader ， b 是 resource path ，'/Users/a/github/html-webpack-plugin-main/lib/loader.js!/Users/a/github/zstack/local-test/src/reactjs/index.html'
						unresolvedResource = rawElements.pop(); // 原始文件路径
						elements = rawElements.map(identToLoaderRequest);//xxx?bbb === {loader:xxx , options: bbb} // loader path
				...
				let loaders;
				// 执行  2 ，然后callback ：[ resolver loader , resolver request file ]
				const continueCallback = needCalls(2, err => {
					...
					try {
						for (const item of loaders) {
							if (typeof item.options === "string" && item.options[0] === "?") {
								...
								item.options = this.ruleSet.references.get(ident); // 获取 loader 的options
					...
					// handle .webpack[] suffix , matchResourceData.resource === '/Users/a/github/webpack-main/webpack-test/src/index.css.webpack[javascript/auto]'
					let resource;
					let match;
					if (
						matchResourceData &&
						...
						const result = this.ruleSet.exec({ //goto 170 RuleSetCompiler  // 从 rule 获取 loader 等数据 , svg:{type: 'type', value: 'asset/resource'}
						...
						for (const r of result) {// rule 只会拿到 effect value ，或者 loader
							if (r.type === "use") {
								if (!noAutoLoaders && !noPrePostAutoLoaders) { // noAutoLoaders === false ， 插入loader ， 否则不使用 loader
									useLoaders.push(r.value);//{loader:css-loader,options,ident}
								}
					...
					const continueCallback = needCalls(3, err => {
						...
						let type = settings.type; // svg : asset/resource
						...
						try {
							Object.assign(data.createData, {
								...
								request: stringifyLoadersAndResource( // 拼接 loader 和 resource === loader1!loader2!resource
									allLoaders,
					...
					this.resolveRequestArray( // 处理 rule 对应的 loaders
						contextInfo,
						this.context,
						useLoadersPost,
						loaderResolver,
				...
				// 从request 获取 loaders `./loader1?xyz!loader2!./resource?rrr'` , 处理request 的 loader
				this.resolveRequestArray(
					contextInfo,
					...
					loaderResolver,
					resolveContext,
					(err, result) => { // {loader: xxx path}
						if (err) return continueCallback(err);
						loaders = result; // loader path to loader options
						continueCallback();
					}
				);

				const defaultResolve = context => { // 处理 request 对应的文件data
					...
					// resource without scheme and with path
						...
						const normalResolver = this.getResolver( // resolver esm files
							"normal", // dependencyType == esm
							dependencyType
								? cachedSetProperty(
						...
						this.resolveResource( // 获取文件
							...
							(err, resolvedResource, resolvedResourceResolveData) => {
								...
								if (resolvedResource !== false) {
									resourceData = {
										resource: resolvedResource, // resource path
				...
				// resource with scheme , request === data:text/javascript,__webpack_public_path__ = __webpack_base_uri__ = htmlWebpackPluginPublicPath;
				if (scheme) { // scheme === [data , file , ...]
					resourceData = {
						resource: unresolvedResource,
						data: {},
						path: undefined,
	...
	create(data, callback) {
		...
		const request = dependency.request; // 一个依赖的 request
		...
		this.hooks.beforeResolve.callAsync(resolveData, (err, result) => {// no plugin
			if (err) {
				return callback(err, {
					fileDependencies,
					missingDependencies,
			...
			this.hooks.factorize.callAsync(resolveData, (err, module) => { // ExternalModuleFactoryPlugin , NormalModuleFactory
				if (err) {
					return callback(err, {
						fileDependencies,
						missingDependencies,
...
If changing the source code is not an option there is also a resolve options called 'preferRelative' which tries to resolve these kind of requests in the current directory too.`
	...
	// map(type , map (parserOptions , parser))
