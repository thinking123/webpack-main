...
class FlagDependencyUsagePlugin {
	...
	/** 使用 module 的 connection 获取module 之间的 exportInfo 引用，设置 runtime exportinfo ，main
	 * Apply the plugin
				...
				modules => {
							...
							for (const usedExportInfo of usedExports) {
								...
								if (usedExport.length === 0) {
									...
									for (let i = 0; i < usedExport.length; i++) {
										...
										if (
											exportInfo.setUsedConditionally( //module ,  exportInfo , 在runtime 使用 (module1 -> module2) ,module1 import module2 exportInfo,module2 exportInfo runtime Used
												v => v !== UsageState.Used,
												UsageState.Used,
												runtime
											)
					...
					const processModule = (module, runtime, forceSideEffects) => {
						...
						for (;;) {
							...
							for (const dep of block.dependencies) { // 从 connection 获取 module1 ref module2  的 exportInfo
								...
								if (
									...
									map.set(module, referencedExports); // module 导出 引用的 ids , import {usedFunction} from 'module' , export {usedFunction ,xxx}
								} else if (
									oldReferencedExports !== undefined &&
