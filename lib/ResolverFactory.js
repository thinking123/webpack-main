...
module.exports = class ResolverFactory {
	...
	/** 三级缓存 map ( type , map (  resolveOptions or JSON.stringify(resolveOptions)  ,  value ))
	 * @param {string} type type of resolver
	 * @param {ResolveOptionsWithDependencyType=} resolveOptions options
	 * @returns {ResolverWithOptions} the resolver
	 */
	get(type, resolveOptions = EMPTY_RESOLVE_OPTIONS) { //EMPTY_RESOLVE_OPTIONS = {}
	...
	_create(type, resolveOptionsWithDepType) {
		...
		); // 创建文件 resolve ： https://github.com/webpack/enhanced-resolve
		...
		this.hooks.resolver // plugin ResolverCachePlugin
			.for(type)
