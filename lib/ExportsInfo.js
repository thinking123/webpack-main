...
class ExportsInfo {
	...
	setHasProvideInfo() {
		...
		if (this._redirectTo !== undefined) {
			...
			if (this._otherExportsInfo.provided === undefined) { // undefined 没有定义 ， undefined: it was not determined if it is provided
				this._otherExportsInfo.provided = false; // false: it is not provided
			}
			if (this._otherExportsInfo.canMangleProvide === undefined) {
				this._otherExportsInfo.canMangleProvide = true; // 是否可以重整提供 ，  true: it can be mangled ： 能够导出重整
			}
		}
	}
...
class ExportInfo {
	...
	constructor(name, initFrom) {
		...
		this._usedInRuntime = //是否在runtime 使用
		...
		if (runtime === undefined) {
			...
			if (newValue !== UsageState.Unused && condition(UsageState.Unused)) {
				...
				forEachRuntime(runtime, runtime =>
					this._usedInRuntime.set(runtime, newValue) // 设置 export 在 runtime 使用 value(UsageState)
				);
				return true;
			}
		} else {
	...
	getUsed(runtime) {
		...
		if (this._usedInRuntime === undefined) {
			return UsageState.Unused; // no used in import module ,return Unused
			...
			for (const value of this._usedInRuntime.values()) { // _usedInRuntime <runtime:main , Used> , used in import module :
