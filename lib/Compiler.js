...
class Compiler {
	...
	getCache(name) {
		return new CacheFacade(
			...
			this.options.output.hashFunction // md4
		);
	}
	...
	run(callback) {
		...
		const onCompiled = (err, compilation) => {
			...
			if (this.hooks.shouldEmit.call(compilation) === false) {// no plugin
...
${other}`);
		...
		this.hooks.emit.callAsync(compilation, err => {// no plugin
			if (err) return callback(err);
			outputPath = compilation.getPath(this.outputPath, {}); // 获取输出文件夹name
	...
	newCompilation(params) {
		...
		this.hooks.compilation.call(compilation, params); // EntryPlugin , DefinePlugin
		return compilation;
	}
	...
	createNormalModuleFactory() {
		...
		this.hooks.normalModuleFactory.call(normalModuleFactory);//no plugin
  ...
  // 编译参数
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => { // no plugin
			if (err) return callback(err);
			...
			logger.time("make hook");//make 构建source的源码和依赖源码
			this.hooks.make.callAsync(compilation, err => {
				logger.timeEnd("make hook");
				if (err) return callback(err);

				logger.time("finish make hook");
				this.hooks.finishMake.callAsync(compilation, err => { // no plugin
					...
					process.nextTick(() => {
						...
						compilation.finish(err => {
							...
							compilation.seal(err => {
								...
								this.hooks.afterCompile.callAsync(compilation, err => { // no plugin
