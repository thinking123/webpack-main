...
exports.getEntryRuntime = (compilation, name, options) => {
	...
	if (dependOn) { // 返回 dependOn runtime
		/** @type {RuntimeSpec} */
...
class RuntimeSpecMap {
	...
	constructor(clone) {
		...
		/** @type {RuntimeSpec} */ //存储key , getRuntimeKey(_singleRuntime)
		this._singleRuntime = clone ? clone._singleRuntime : undefined;
		/** @type {T} */ // 存储value , mode === 1
		this._singleValue = clone ? clone._singleValue : undefined;
		/** @type {Map<string, T> | undefined} */ // 存储 value mode === 2，map(getRuntimeKey(_singleRuntime) , v)
