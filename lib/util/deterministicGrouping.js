...
/**
 * 如果 a，b 字符串 大部分类似 dist > 0 ,否则返回 0
 * @param {string} a key
 * @param {string} b key
...
// 只需要一个 size type > maxSize
const isTooBig = (size, maxSize) => { // {xx:size} some key >  {xx:size}
	for (const key of Object.keys(size)) {
		const s = size[key];
		if (s === 0) continue;
...
class Node {
	...
	constructor(item, key, size) {
		this.item = item; // NormalModule
		this.key = key; // webpack-test_src_commonjs_common1_js-50c90b24
		this.size = size;// {javascript:60}
	}
}
...
class Group {
	...
	/** 删除 不符合 filter 的 nodes ，重新计算 similarities 和 size
...
const getSimilarities = nodes => {
	...
	for (const node of nodes) {
		if (last !== undefined) {
			similarities.push(similarity(last.key, node.key)); // 计算key 的 显示度 ： add
		}
		last = node;
	}
	return similarities;
 ...
 * @returns {GroupedItems<T>[]} grouped items : chunk: modules
 */
module.exports = ({ maxSize, minSize, items, getSize, getKey }) => {
	...
	//[m1 ,m2 ,m3 ,m4] => chunk1(m1 > maxSize) , group (m2 ,m3 ,m4) 继续分组
	// return nodes bigger than maxSize directly as group
	// But make sure that minSize is not violated
	for (const node of nodes) { // 分组 size > maxSize 新的chunk , 粒度最小单位 module
		if (isTooBig(node.size, maxSize) && !isTooSmall(node.size, minSize)) {
			result.push(new Group([node], [])); // 不符合条件： minSize < node size   && node size  > maxSize :
		} else {
			initialNodes.push(node); // 剩余 size < maxSize : 分组到新的 group 继续分组
		}
	}
	...
	if (initialNodes.length > 0) {
		...
		const removeProblematicNodes = (group, consideredSize = group.size) => {
			...
			if (problemTypes.size > 0) {
				...
				const problemNodes = group.popNodes( // module  小于 minSize的列表
				...
				const possibleResultGroups = result.filter( // 从result 获取 符合 problemTypes 的 groups
					n => getNumberOfMatchingSizeTypes(n.size, problemTypes) > 0
				);
				if (possibleResultGroups.length > 0) { // 合并 module  小于 minSize的列表 到 已经分组的 group
					// 获取 max possibleResultGroups 有 problemTypes 属性的
					const bestGroup = possibleResultGroups.reduce((min, group) => {
						...
						if (//比较 min 和 group 对应 problemTypes 的 size 返回 大于的
					...
					result.push(new Group(problemNodes, null)); // 创建 < minSize 的 group
				}
				return true;
			} else {
				return false;
		...
		if (initialGroup.nodes.length > 0) {
			...
			while (queue.length) {
				...
				if (!isTooBig(group.size, maxSize)) { //剩余的 group 不够分组chunk ，分到一个 chunk
					result.push(group);
					continue;
				} // 继续分组split chunk
