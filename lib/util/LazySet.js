...
/** _set 保存迭代数据，_toMerge 保存addAll的 非 LazySet 列表数组 [[],[],...] , _toDeepMerge 保存 LazySet 数组, [...]
	...
	addAll(iterable) {
		if (this._deopt) {
			...
			if (iterable instanceof LazySet) {
				...
				this._toDeepMerge.push(iterable); // 放入 _toDeepMerge [ iterable : LazySet ]
				this._needMerge = true;
				if (this._toDeepMerge.length > 100000) {
					this._flatten();
				}
			} else {
				this._toMerge.add(iterable); // _toMerge [ iterable ]
				this._needMerge = true;
			}
