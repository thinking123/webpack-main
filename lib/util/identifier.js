...
const absoluteToRequest = (context, maybeAbsolutePath) => {
	if (maybeAbsolutePath[0] === "/") { // 绝对路径 `/`
		if (
			maybeAbsolutePath.length > 1 &&
			maybeAbsolutePath[maybeAbsolutePath.length - 1] === "/" // 是 目录 `/a/b/`
		) {
		...
		let resource = // maybeAbsolutePath === /a?q=1  => /a
			querySplitPos === -1
				? maybeAbsolutePath
				: maybeAbsolutePath.slice(0, querySplitPos);// 有 `?` 获取 querystring 之前的path
		resource = relativePathToRequest(path.posix.relative(context, resource));// 返回从context 到 resource 的路径
...
const makeCacheable = fn => {
	...
	/** map(associatedObjectForCache , map ())
	 * @param {Object=} associatedObjectForCache an object to which the cache will be attached
	...
	cachedFn.bindCache = associatedObjectForCache => {
		...
		/** map(context , map(identifier , cachedResult))
		 * @param {string} context context used to create relative path
		 * @param {string} identifier identifier used to create relative path
		 * @returns {string} the returned relative path
		 */
	...
	/** 二级缓存，map(associatedObjectForCache , map ( context , map ( key , value)))
	 * @param {string} context context used to create relative path
	 * @param {Object=} associatedObjectForCache an object to which the cache will be attached
	...
	cachedFn.bindContextCache = (context, associatedObjectForCache) => {
		...
		const boundFn = identifier => {
			...
			if (cachedResult !== undefined) {
				return cachedResult;
			} else { // fn === _contextify ，存储相对 context 路径保存
				const result = fn(context, identifier);
				innerSubCache.set(identifier, result);
				return result;
			}
...
/** //http://www.qq.com?sfsdf#dsddsfsdf , [http://www.qq.com,?sfsdf,#dsddsfsdf]
...
exports.parseResource = (realFn => {
	...
	const cache = new WeakMap();
	// 一级缓存 map(associatedObjectForCache , map(  ))
