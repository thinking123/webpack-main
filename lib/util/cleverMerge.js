...
/** 缓存 map (first, map(second , mergeObj ))
...
/** map(obj , map(property , map(value , {...obj , 	[property]: value}) ))
 * @template T
...
const parseObject = obj => {
	...
	let dynamicInfo;
	const getInfo = p => {
		const entry = info.get(p);// p === key1
		if (entry !== undefined) return entry;
		const newEntry = {
			base: undefined, // not start with by key value = obj[key] === v1
			byProperty: undefined, // @type === string , obj 其他的 byXXX
			byValues: undefined // @type === Map , (key = byXXX 的keys , value = byXXX 的value  的 key 的value) ，例如 obj = {key1:v1 , byXXX: { byValue: { byVKey1(等于key1):byVV1 } }} ,map === (byValue,byVV1) ,也就是不是by开头的key1 在byXXX的 里面有相同的key(byVKey1)
		};
		info.set(p, newEntry);
		return newEntry;
	};
	for (const key of Object.keys(obj)) {
		if (key.startsWith("by")) { // obj key start with byXXX(obj key)
			const byProperty = key;
			const byObj = obj[byProperty]; // byObj == byXXX value
			if (typeof byObj === "object") { // byXXX value === {}
				for (const byValue of Object.keys(byObj)) { // list byXXX value -> {byXXX: {key1 : value1}}
					const obj = byObj[byValue]; // obj === obj{byXXXkey1:{key1: key1value1 === obj }} , key1value1 === current obj
					for (const key of Object.keys(obj)) { // list key1value1 === {key11: key11value11}
						const entry = getInfo(key); // this key11 === not start with byxxxx entry's key
						if (entry.byProperty === undefined) {
							entry.byProperty = byProperty; // obj key start with byXXX(obj key)
						...
						} // {byXXX:{ byValue : { key === top obj key :  getInfo(key) } === obj }}
		...
		} else { // key(obj key)  不是 startwith by set to info<map>(key , { base: keyValue( obj value) , byProperty , byValues })
			const entry = getInfo(key);
			entry.base = obj[key];
		}
	}
...
const serializeObject = (info, dynamicInfo) => {
	...
	for (const entry of info.values()) {
		...
		} // obj ===  { byXXX : { byValue,byVV1 } }
	}
	for (const [key, entry] of info) {
		if (entry.base !== undefined) {
			obj[key] = entry.base; // obj ===  { byXXX : { byValue,byVV1 } , key : key not start with By value }
		}
...
const _cleverMerge = (first, second, internalCaching = false) => {
	...
	for (const [key, firstEntry] of firstInfo) { // key : 不是by开头 , firstEntry : map (byValue,byVV1)
...
const removeOperations = obj => {
	...
	for (const key of Object.keys(obj)) {
		...
		switch (type) {
			case VALUE_TYPE_UNDEFINED:
			case VALUE_TYPE_DELETE:
				break; //删除  Symbol("DELETE") 和 undefined key
			case VALUE_TYPE_OBJECT:
				newObj[key] = removeOperations(value);
				break;
			case VALUE_TYPE_ARRAY_EXTEND:
				newObj[key] = value.filter(i => i !== "..."); // 删除  ...
				break;
			default:
				newObj[key] = value;
				break;
