...
class AsyncQueue {
	 ...
	 * @param {AsyncQueue<any, any, any>=} options.parent parent queue, which will have priority over this queue and with shared parallelism , child 先执行parent list tasks ，然后执行 child tasks
	...
	constructor({ name, parallelism, parent, processor, getKey }) {
		...
		this._stopped = false;
		this._root = parent ? parent._root : this; // root=== this or parent this
		if (parent) {
	 ...
	 * @returns {void}
	 */
	_ensureProcessing() { // 当前运行 task 和 最大并行数量
		while (this._activeTasks < this._parallelism) {
			const entry = this._queued.dequeue(); // pop queue task
			if (entry === undefined) break;
			this._activeTasks++;
			entry.state = PROCESSING_STATE; // task status
			this._startProcessing(entry); // start task
		}
		this._willEnsureProcessing = false;
		if (this._queued.length > 0) return; // 为空处理 _children processor
		if (this._children !== undefined) { // call _children processor
