...
class JavascriptModulesPlugin {
			...
			(compilation, { normalModuleFactory }) => {
				...
				compilation.hooks.renderManifest.tap(
					...
					(result, options) => {
						...
						let render; //filenameTemplate == '[name].[contenthash].js'
				...
				compilation.hooks.chunkHash.tap(
					...
					(chunk, hash, context) => {
						...
						if (chunk.hasRuntime()) {
							this.updateHashWithBootstrap( // renderBootstrap
								hash,
								{
									hash: "0000",
									chunk,
				...
				compilation.hooks.contentHash.tap("JavascriptModulesPlugin", chunk => {
					...
					if (chunk.hasRuntime()) {
						this.updateHashWithBootstrap( // 合并计算：计算 bootstrap hash
							hash,
							{
								hash: "0000",
								chunk,
					...
					if (modules) { // 合并计算： javascript module hash
					...
					if (runtimeModules) {//合并计算： runtime module （runtime hash 计算）hash
						const xor = new StringXor();
					...
					chunk.contentHash.javascript = digest.substr(0, hashDigestLength); // js hash
				});
	...
	renderMain(renderContext, hooks, compilation) {
		...
		const useSourceMap = hooks.useSourceMap.call(chunk, renderContext); //no plugin , dev-tool : sourcemap
	...
	// 添加 chunk 和 normal module 代码
		...
		let finalSource = tryRunOrWebpackError(
			() => hooks.renderMain.call(source, renderContext), // no plugin
		...
		finalSource = tryRunOrWebpackError(
			() => hooks.renderContent.call(finalSource, renderContext),  // no plugin
			"JavascriptModulesPlugin.getCompilationHooks().renderContent"
		);
		if (!finalSource) {
			throw new Error(
