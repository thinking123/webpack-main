...
class JavascriptParser extends Parser {
			...
			this.hooks.evaluate.for(exprType).tap("JavascriptParser", expr => {
				...
				if (info !== undefined) {
					return this.callHooksForInfoWithFallback(
						...
						name => {
							const hook = this.hooks.evaluateDefinedIdentifier.get(name); // evaluateDefinedIdentifier no plugins
							if (hook !== undefined) {
	...
	preWalkStatement(statement) {
		...
		if (this.hooks.preStatement.call(statement)) {// FunctionDeclaration === __webpack_require__
	...
	blockPreWalkStatement(statement) {
		this.statementPath.push(statement);
		if (this.hooks.blockPreStatement.call(statement)) {// no plugins
			this.prevStatement = this.statementPath.pop();
			return;
		}
		switch (statement.type) {
	...
	walkStatement(statement) {
		this.statementPath.push(statement);
		if (this.hooks.statement.call(statement) !== undefined) { // no plugins
			this.prevStatement = this.statementPath.pop();
			return;
		}
		switch (statement.type) {
	...
	// import 的时候 , import {a as b} , localName === b , importedName === a
	blockPreWalkImportDeclaration(statement) {
		const source = statement.source.value;// import {fun} from './other.js', source === other.js
		this.hooks.import.call(statement, source);
		for (const specifier of statement.specifiers) {
			const name = specifier.local.name; // name === fun
			switch (specifier.type) {
				case "ImportDefaultSpecifier":
					...
					break;
				case "ImportSpecifier": // import {util1 as util2} from 'fs.js'
					if (
						!this.hooks.importSpecifier.call(
							statement,
							source, // fs.js
							specifier.imported.name, // === util1 ， importedName
							name // util2 ， localName
						)
					) { // importSpecifier  已经tag
						this.defineVariable(name);
					}
					break;
	...
	blockPreWalkExportNamedDeclaration(statement) {
		...
		if (statement.source) {
			...
			this.hooks.exportImport.call(statement, source); // export {xx} from 'source'
		} else {
			this.hooks.export.call(statement); // export {a , b}
		}
		...
		if (statement.specifiers) {//  export {a , b}, specifiers === [a, b]
			for (
				...
				switch (specifier.type) {
					case "ExportSpecifier": {
						...
						if (source) {
							...
							this.hooks.exportSpecifier.call( // export { a as fun} , localName = a , name === fun
								statement,
								specifier.local.name,
								name,
								specifierIndex
	...
	blockPreWalkVariableDeclaration(statement) {
		...
		const hookMap =
			statement.kind === "const"
				? this.hooks.varDeclarationConst // maps === 0
				: this.hooks.varDeclarationLet; // maps === 0
	...
	_preWalkVariableDeclaration(statement, hookMap) {
		for (const declarator of statement.declarations) {
			switch (declarator.type) {
				case "VariableDeclarator": {
					if (!this.hooks.preDeclarator.call(declarator, statement)) { // no plugins
		...
		for (const declarator of statement.declarations) {
			switch (declarator.type) {
				case "VariableDeclarator": {
					...
					if (!this.hooks.declarator.call(declarator, statement)) { // declarator no plugins
	...
	callHooksForInfoWithFallback(hookMap, info, fallback, defined, ...args) {
		...
		if (typeof info === "string") {
			...
			while (tagInfo !== undefined) {
				const hook = hookMap.get(tagInfo.tag); // 从 tag call
				if (hook !== undefined) {
		...
		const hook = hookMap.get(name); // 从 name call
		if (hook !== undefined) {
			const result = hook.call(...args);
			if (result !== undefined) return result;
		}
	...
	enterIdentifier(pattern, onIdent) {
		if (!this.callHooksForName(this.hooks.pattern, pattern.name, pattern)) {// pattern __webpack_require__ plugin
	...
	parse(source, state) {
		...
		if (this.hooks.program.call(ast, comments) === undefined) {
			...
			this.prevStatement = undefined;
			this.blockPreWalkStatements(ast.body); //  tag import , tag var and scope : 获取import 和 var
			this.prevStatement = undefined;
	...
	/** automatic semicolon insertion: https://stackoverflow.com/questions/2846283/what-are-the-rules-for-javascripts-automatic-semicolon-insertion-asi
	 ...
	 */
	getMemberExpressionInfo(expression, allowedTypes) {//a.b.c => {a , [c,b]}
		...
		switch (object.type) {
			...
			case "ThisExpression": {
				if ((allowedTypes & ALLOWED_MEMBER_TYPES_EXPRESSION) === 0)
					return undefined;
				const rootName = getRootName(object); // c.log  , root === c
				if (!rootName) return undefined;
