...
const contextifySourceMap = (context, sourceMap, associatedObjectForCache) => {
	...
	return {
		...
		sources: newSources // ['webpack://./webpack-test/a.js']
	};
};
...
class NormalModule extends Module {
	...
	/** buildMeta , buildInfo.exportsArgument : ast data
	...
	_createLoaderContext(resolver, options, compilation, fs, hooks) {
		...
		const loaderContext = {
			...
			getResolve(options) {// less loader resolver alias
	...
	createSource(context, content, sourceMap, associatedObjectForCache) {
		...
		if (this.useSourceMap && sourceMap) { // 对 loader 的 sources 处理: ['/Users/a/github/webpack-main/webpack-test/a.js'] => ['webpack://./webpack-test/a.js']
			return new SourceMapSource(
				content,
	...
	_doBuild(options, compilation, resolver, fs, hooks, callback) {
		...
		try {
			hooks.beforeLoaders.call(this.loaders, this, loaderContext);// no plugin
		} catch (err) {
			processResult(err);
			return;
		}
		...
		runLoaders(
			{
				...
				processResource: (loaderContext, resourcePath, callback) => {
					const resource = loaderContext.resource; // get file content
	...
	_initBuildHash(compilation) {
		...
		if (this._source) {
			hash.update("source");
			this._source.updateHash(hash);// update Source Type string (RawSource) and source.toString()
		}
		hash.update("meta");
	...
	build(options, compilation, resolver, fs, callback) {
		...
		// 获取source ：使用 loader 处理之后的resource 文件 ， ast 分析文件
		return this._doBuild(options, compilation, resolver, fs, hooks, err => {
			...
			const handleParseResult = result => {
				this.dependencies.sort(
					concatComparators(
						compareSelect(a => a.loc, compareLocations), // 比较 location
						keepOriginalOrder(this.dependencies) // 比较 index
					)
				);
				this._initBuildHash(compilation);
			...
			const handleBuildDone = () => {
				try {
					hooks.beforeSnapshot.call(this);// no plugin
				} catch (err) {
					this.markModuleAsErrored(err);
					return callback();
				}
				...
				const checkDependencies = deps => {
					for (const dep of deps) {
						if (!ABSOLUTE_PATH_REGEX.test(dep)) { // windows path: d:p/p or /xxx/xx === 绝对路径
							if (nonAbsoluteDependencies === undefined)
								nonAbsoluteDependencies = new Set();
							nonAbsoluteDependencies.add(dep);
							deps.delete(dep);
							try {
								const depWithoutGlob = dep.replace(/[\\/]?\*.*$/, ""); // \*sdfsdf
								const absolute = join(
			...
			try {
				hooks.beforeParse.call(this);// no plugin
			} catch (err) {
				this.markModuleAsErrored(err);
				this._initBuildHash(compilation);
				return callback();
			...
			try {
				const source = this._source.source();
				result = this.parser.parse(this._ast || source, { // state
					source,
					current: this,
					module: this,
	...
	codeGeneration({
		...
		const resultEntry = { //  按照 sourde 的type 保存 :javascript
			sources,
			runtimeRequirements, // runtime 运行关键字
			data
		};
		return resultEntry;
	}
	...
	needBuild(context, callback) {
		...
		// build if enforced
		if (this._forceBuild) return callback(null, true); // 比较缓存 是否需要 build
