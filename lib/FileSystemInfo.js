...
class FileSystemInfo {
	...
	constructor(
		fs,
		{
			managedPaths = [],//An array of paths that are managed by a package manager and can be trusted to not be modified otherwise.
			immutablePaths = [],//An array of paths that are managed by a package manager and contain a version or a hash in their paths so that all files are immutable.
			logger,
			hashFunction = "md4"
		} = {}
	) {
		...
		this._fileTimestamps = new StackedCacheMap(); // map(文件路径 , {缓存时间，修改时间})
